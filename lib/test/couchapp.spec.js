(function() {
  ...
  describe("couchApp", function() {
    ...
    describe(".sign_up(email, password)", function() {
      ...
      it("should send a PUT request to http://my.cou.ch/_users/org.couchdb.user:email", function() {
        expect($.ajax).wasCalled();
        return expect(this.args.type).toBe('PUT');
      });
    ...
    return describe(".store", function() {
      ...
      describe(".get(type, id)", function() {
        ...
        _when("object can be found", function() {
          beforeEach(function() {
            spyOn(this.app.store, "cache").andReturn({
              name: 'test'
            });
        ...
        _when("object cannot be found", function() {
          beforeEach(function() {
            spyOn(this.app.store, "cache").andReturn(false);
            this.promise = this.app.store.get('document', 'abc4567');
      ...
      describe(".getAll(type)", function() {
        ...
        with_2_cats_and_3_dogs = function(specs) {
          return _and("two cat and three dog objects exist in the store", function() {
            beforeEach(function() {
              spyOn(this.app.store, "_index").andReturn(["cat/1", "cat/2", "dog/1", "dog/2", "dog/3"]);
              return spyOn(this.app.store, "cache").andReturn({
                name: 'becks'
              });
            });
            return specs();
        ...
        it("should return a promise", function() {
          var promise;
          promise = this.app.store.getAll('document');
          expect(promise.done).toBeDefined();
          return expect(promise.fail).toBeDefined();
        });
        _when("called without a type", function() {
          ...
          _and("no documents exist in the store", function() {
            beforeEach(function() {
              return spyOn(this.app.store, "_index").andReturn([]);
            });
          ...
          return _and("there are other documents in localStorage not stored with app.store", function() {
            beforeEach(function() {
              spyOn(this.app.store, "_index").andReturn(["_some_config", "some_other_shizzle", "whatever", "valid/123"]);
              return spyOn(this.app.store, "cache").andReturn({});
            });
        ...
        return _when("called with type = 'cat'", function() {
          return with_2_cats_and_3_dogs(function() {
            return it("should return only the cat objects", function() {
              var promise, results, success;
              success = jasmine.createSpy('success');
              promise = this.app.store.getAll('cat');
              promise.done(success);
              results = success.mostRecentCall.args[0];
      ...
      describe(".destroy(type, id)", function() {
        it("should return a promise", function() {
          var promise;
          promise = this.app.store.destroy('document', '123');
          expect(promise.done).toBeDefined();
          return expect(promise.fail).toBeDefined();
        });
        return it("should have more specs");
      });
      ...
      return describe(".clear_changed()", function() {
        return it("should have some specs");
      });
    });
  });
